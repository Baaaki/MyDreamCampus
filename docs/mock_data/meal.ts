...
export const getAvailableSlots = (
  ...
  const capacity = cafeteria.name.includes("Merkez")
    ? 500
    : cafeteria.name.includes("Mühendislik")
      ? 300
      : 200

  const reservations = mockMyReservationsResponse.reservations.filter(
    (r) =>
      r.cafeteria?.id === cafeteriaId &&
      r.date === date &&
      r.status === "confirmed"
  )

  const lunchCount = reservations.filter((r) => r.meal_time === "lunch").length
  const dinnerCount = reservations.filter(
    (r) => r.meal_time === "dinner"
  ).length
