...
function mapCourseResponseToCourseCatalog(
...
export const catalogService = {
  ...
  ): Promise<{ courses: CourseCatalog[]; pagination: PaginationResponse }> {
  ...
  async getCourseByCode(courseCode: string): Promise<CourseCatalog> {
    const response = await catalogApi
      .get(`courses/${encodeURIComponent(courseCode)}`)
      .json<CourseResponse>()
  ...
  async updateCourse(
    ...
    const response = await catalogApi
      .put(`courses/${encodeURIComponent(courseCode)}`, { json: data })
      .json<CourseResponse>()
    return mapCourseResponseToCourseCatalog(response)
  },
