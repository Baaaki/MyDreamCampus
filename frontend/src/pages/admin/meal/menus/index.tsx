...
function MealAutocomplete({
  ...
  const filteredMeals = mealDatabase
    .filter((meal) => {
    ...
    <div ref={containerRef} className="relative">
      <div className="flex items-center">
        <Input
          ...
          disabled={disabled}
          className={`h-8 border-0 bg-transparent pr-6 text-center text-xs uppercase focus:bg-white dark:focus:bg-gray-800 ${disabled ? "cursor-not-allowed opacity-60" : ""}`}
        />
        {searchTerm && !disabled && (
          <button
            onClick={handleClear}
        ...
        <div className="absolute z-50 mt-1 max-h-48 w-full overflow-y-auto rounded-md border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800">
          {filteredMeals.map((meal, idx) => (
            <button
              ...
              className="flex w-full items-center justify-between px-2 py-1.5 text-left text-xs hover:bg-gray-100 dark:hover:bg-gray-700"
            >
...
function VeganMealAutocomplete({
  value,
  onChange,
  placeholder,
  categoryFilter,
  ...
  const filteredMeals = veganMealDatabase
    .filter((meal) => {
      const matchesSearch = meal.name.includes(searchTerm.toUpperCase())
      const matchesCategory =
        !categoryFilter || meal.category === categoryFilter
...
const getCategoryByIndex = (
  index: number
): "soup" | "main" | "side" | "dessert" | "other" => {
  const categories: ("soup" | "main" | "side" | "dessert" | "other")[] = [
    "soup",
    "main",
    "side",
    "dessert",
...
export const sampleWeeklyMenu: WeeklyMenu = {
  monday: {
    items: sampleMenuItems.monday,
    calories: calcCalories(sampleMenuItems.monday),
  },
  tuesday: {
    items: sampleMenuItems.tuesday,
    calories: calcCalories(sampleMenuItems.tuesday),
  },
  wednesday: {
    items: sampleMenuItems.wednesday,
    calories: calcCalories(sampleMenuItems.wednesday),
  },
  thursday: {
    items: sampleMenuItems.thursday,
    calories: calcCalories(sampleMenuItems.thursday),
  },
  friday: {
    items: sampleMenuItems.friday,
    calories: calcCalories(sampleMenuItems.friday),
  },
}
...
export const sampleVeganWeeklyMenu: WeeklyMenu = {
  monday: {
    items: sampleVeganMenuItems.monday,
    calories: calcVeganCalories(sampleVeganMenuItems.monday),
  },
  tuesday: {
    items: sampleVeganMenuItems.tuesday,
    calories: calcVeganCalories(sampleVeganMenuItems.tuesday),
  },
  wednesday: {
    items: sampleVeganMenuItems.wednesday,
    calories: calcVeganCalories(sampleVeganMenuItems.wednesday),
  },
  thursday: {
    items: sampleVeganMenuItems.thursday,
    calories: calcVeganCalories(sampleVeganMenuItems.thursday),
  },
  friday: {
    items: sampleVeganMenuItems.friday,
    calories: calcVeganCalories(sampleVeganMenuItems.friday),
  },
}
...
export default function MenusPage() {
  ...
  const [menuStatus, setMenuStatus] = useState<"empty" | "saved" | "editing">(
    "empty"
  )
  ...
  const [savedWeeklyMenus, setSavedWeeklyMenus] = useState<WeeklyMenu[] | null>(
    null
  )
  const [savedVeganWeeklyMenus, setSavedVeganWeeklyMenus] = useState<
    WeeklyMenu[] | null
  >(null)
  ...
  const isFutureMonth = useMemo(() => {
    ...
    const maxEditableDate = new Date(
      currentDate.getFullYear(),
  ...
  useEffect(() => {
    if (existingMenu) {
      ...
      setSavedWeeklyMenus(
        JSON.parse(JSON.stringify(existingMenu.menu_data.normalMenus))
      )
      setSavedVeganWeeklyMenus(
        JSON.parse(JSON.stringify(existingMenu.menu_data.veganMenus))
      )
      setMenuStatus("saved")
      setIsEditMode(false)
  ...
  const calculateDayCalories = (items: string[], isVegan = false): number => {
    return items.reduce((total, item) => {
      ...
      return (
        total + (isVegan ? getVeganMealCalories(item) : getMealCalories(item))
      )
    }, 0)
  }
  ...
  const updateMenuItem = (
    ...
    setWeeklyMenus((prev) => {
      const updated = [...prev]
      const newItems = [...updated[weekIndex][day].items] as [
        string,
        string,
        string,
        string,
          ...
          <p className="text-muted-foreground">
            ...
            {isPastMonth && (
              <span className="ml-2 text-amber-600">
                (Geçmiş ay - salt okunur)
              </span>
            )}
            {isFutureMonth && (
              <span className="ml-2 text-purple-600">
                (İleri tarih - henüz düzenlenemez)
              </span>
            )}
          </p>
        </div>
        ...
        <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-900/20">
          <div className="flex items-center gap-3">
            ...
            <p className="text-amber-800 dark:text-amber-300">
              Bu menü geçmiş bir aya ait olduğu için yalnızca görüntülenebilir.
              Düzenleme yapılamaz.
            </p>
          </div>
        </div>
          ...
          <div className="rounded-lg border border-green-300 bg-green-50 p-4 dark:border-green-800 dark:bg-green-900/20">
            <div className="flex items-center gap-3">
              ...
              <p className="text-green-800 dark:text-green-300">
                {selectedMonthLabel} {selectedYear} için henüz menü
                oluşturulmamış. <strong>Oluştur</strong> butonuna tıklayarak
                yeni menü oluşturmaya başlayabilirsiniz.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="rounded-lg border border-gray-300 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/20">
          <div className="flex items-center gap-3">
            ...
            <p className="text-gray-800 dark:text-gray-300">
              {selectedMonthLabel} {selectedYear} için menü oluşturulmamış.
              Geçmiş aylar için yeni menü oluşturulamaz.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="rounded-lg border border-purple-300 bg-purple-50 p-4 dark:border-purple-700 dark:bg-purple-900/20">
          <div className="flex items-center gap-3">
            ...
            <p className="text-purple-800 dark:text-purple-300">
              {selectedMonthLabel} {selectedYear} için henüz menü oluşturulamaz.
              Sadece mevcut ay ve sonraki ay için menü düzenleyebilirsiniz.
            </p>
          </div>
        </div>
      )}
            ...
            <div className="flex-1">
              ...
              <div className="max-h-40 space-y-2 overflow-y-auto">
                {pendingChanges.map((change, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between rounded border border-orange-200 bg-white p-2 text-sm dark:border-orange-700 dark:bg-gray-800"
                  >
                    <div className="flex-1">
                      <span className="font-medium text-orange-700 dark:text-orange-300">
                        {change.isVegan ? "🥗 Vegan" : "🍖 Normal"} -{" "}
                        {change.dateString} ({change.dayLabel})
                      </span>
                    ...
                    <button
                      ...
                      className="ml-2 rounded p-1 hover:bg-orange-100 dark:hover:bg-orange-800"
      ...
      {menuStatus === "saved" && !isEditMode && (
        <div className="rounded-lg border border-blue-300 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
          <div className="flex items-center gap-3">
            ...
            <p className="text-blue-800 dark:text-blue-300">
              Menü kilitli. Değişiklik yapmak için <strong>Düzenle</strong>{" "}
              butonuna tıklayın.
            </p>
          </div>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="normal">
            <Card className="overflow-hidden dark:border-gray-800 dark:bg-gray-900">
              <CardHeader className="bg-blue-900 py-4 text-center text-white">
                <div className="flex items-center justify-center gap-4">
                  ...
                  <div>
                    <h2 className="text-lg font-bold">
                      MYDREAMCAMPUS ÜNİVERSİTESİ
                    </h2>
                    <h3 className="text-sm">
                      SAĞLIK KÜLTÜR VE SPOR DAİRE BAŞKANLIĞI
                    </h3>
                    <h4 className="mt-1 text-base font-semibold">
                      {selectedYear} {selectedMonthLabel.toUpperCase()} AYI -
                      YEMEK LİSTESİ
                    </h4>
                  </div>
                </div>
                          ...
                          <thead>
                            <tr className="bg-yellow-100 dark:bg-yellow-900/30">
                              {dayNames.map((day, idx) => (
                                <th
                                  key={day.key}
                                  className="min-w-[180px] border border-gray-300 p-2 text-center text-sm font-semibold text-blue-900 dark:border-gray-600 dark:text-blue-300"
                                >
                                ...
                                {dayNames.map((day) => (
                                  ...
                                  >
                                    <MealAutocomplete
                                      ...
                                      categoryFilter={getCategoryByIndex(
                                        rowIdx
                                      )}
          ...
          <TabsContent value="vegan">
            <Card className="overflow-hidden dark:border-gray-800 dark:bg-gray-900">
              <CardHeader className="bg-green-800 py-4 text-center text-white">
                <div className="flex items-center justify-center gap-4">
                  <img
                    src="/mdc-logo.png"
                    alt="MDC"
                          ...
                          <thead>
                            <tr className="bg-green-100 dark:bg-green-900/30">
                              {dayNames.map((day, idx) => (
                                <th
                                  key={day.key}
                                  className="min-w-[180px] border border-gray-300 p-2 text-center text-sm font-semibold text-green-900 dark:border-gray-600 dark:text-green-300"
                                >
                                  <div>{weekDates[idx]}</div>
                                  <div className="text-xs font-normal text-gray-600 dark:text-gray-400">
                                    {day.label}
      ...
      <Card className="dark:border-gray-800 dark:bg-gray-900">
        <CardContent className="pt-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            *Yukarıda belirtilen 1 öğünlük toplam kalori değerlerine, 50 gr
            ekmeğin değeri olan 136 kalori ilave edilmiştir.
          </p>
        </CardContent>
      </Card>
