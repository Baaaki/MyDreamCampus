...
export default function StudentMealReservationPage() {
  ...
  const [selectedMenuType, setSelectedMenuType] = useState<"normal" | "vegan">(
    "normal"
  )
  ...
  const handleReserve = async () => {
    ...
    if (!isReservationWindowOpen()) {
      setError(
        "Rezervasyon sadece Pazartesi 08:00 - Cuma 13:00 arası yapılabilir"
      )
      return
    }
    ...
    try {
      ...
      setSuccess(
        "Rezervasyon başarıyla oluşturuldu. Ödeme sayfasına yönlendiriliyorsunuz..."
      )
  ...
  const handleCancelReservation = async (reservationId: string) => {
    ...
    if (!isReservationWindowOpen()) {
      alert(
        "Rezervasyon iptali sadece Pazartesi 08:00 - Cuma 13:00 arası yapılabilir"
      )
      return
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="mx-auto max-w-4xl px-4">
        ...
        <div
          className={`mb-6 rounded-lg p-4 ${isReservationWindowOpen() ? "bg-green-50" : "bg-red-50"}`}
        >
          <p
            className={`text-sm ${isReservationWindowOpen() ? "text-green-800" : "text-red-800"}`}
          >
        ...
        <div className="mb-6 rounded-lg bg-white p-6 shadow-md">
          ...
          <div className="space-y-4">
              ...
              <div className="flex gap-4">
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      setSelectedMealTime(e.target.value as "lunch")
                    }
                    className="mr-2"
                    disabled={!isReservationWindowOpen()}
                  />
                ...
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      setSelectedMealTime(e.target.value as "dinner")
                    }
                    className="mr-2"
                    disabled={!isReservationWindowOpen()}
                  />
              ...
              <div className="flex gap-4">
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      setSelectedMenuType(e.target.value as "normal")
                    }
                    className="mr-2"
                    disabled={!isReservationWindowOpen()}
                  />
                ...
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      setSelectedMenuType(e.target.value as "vegan")
                    }
                    className="mr-2"
                    disabled={!isReservationWindowOpen()}
                  />
            ...
            <button
              ...
              className="rounded-md bg-indigo-600 px-6 py-2 font-medium text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50"
            >
                ...
                <div key={reservation.id} className="rounded-lg border p-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600">
                        Öğün:{" "}
                        {reservation.meal_time === "lunch" ? "Öğle" : "Akşam"} |
                        Menü:{" "}
                        {reservation.menu_type === "normal"
                          ? "Normal"
                          : "Vegan"}
                      </p>
                    ...
                    {!reservation.is_used &&
                      reservation.status === "confirmed" && (
                        <button
                          ...
                          disabled={!isReservationWindowOpen()}
                          className="ml-4 rounded-md bg-red-100 px-4 py-2 text-sm font-medium text-red-700 hover:bg-red-200 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                          İptal Et
                        </button>
                      )}
