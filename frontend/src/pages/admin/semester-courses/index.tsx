...
const createSemesterCourse = async (
  semester: string,
  data: CreateSemesterCourseRequest
) => {
...
export default function SemesterCoursesPage() {
  ...
  const totalWeight = formData.assessment_schema.reduce(
  ...
  const { data: existingSemesterCourses = [] } = useQuery({
    ...
    queryFn: () =>
      fetchSemesterCourses(semester, getDepartmentName(formData.department_id)),
    enabled: !!formData.department_id,
    staleTime: 5 * 60 * 1000,
  })
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateSemesterCourseRequest) =>
      createSemesterCourse(semester, data),
    onSuccess: () => {
    ...
    onError: async (error: any) => {
      ...
      if (code === "INSTRUCTOR_SCHEDULE_CONFLICT" && details?.department) {
        ...
        const dayTR =
          dayToTurkish[details.day_of_week ?? ""] || details.day_of_week
        setErrorDialog({
          open: true,
        ...
        setErrorDialog({
          ...
          title: "Program Çakışması",
          message:
            "Öğretim görevlisinin bu saatlerde başka bir dersi bulunuyor",
        })
      } else {
    ...
    for (const course of existingSemesterCourses) {
      for (const session of course.schedule_sessions) {
        if (
          ...
          session.slot_numbers.includes(slot)
        ) {
  ...
  const updateAssessment = (
    ...
    setFormData((prev) => ({
      ...
      assessment_schema: prev.assessment_schema.map((item, i) => {
        if (i === index) {
          if (field === "slug") {
            const predefined = predefinedAssessments.find(
              (p) => p.slug === value
            )
            return {
              ...item,
              slug: value as string,
              name: predefined?.name || item.name,
            }
          }
  ...
  const toggleScheduleSlot = (day: string, slot: number) => {
    setFormData((prev) => {
      const key = (s: ScheduleSessionDTO) =>
      ...
      if (!isRemoving) {
        const maxSlots =
          activeSessionType === "theory" ? totalTheoryHours : totalLabHours
      ...
      if (existingSession) {
        ...
        if (hasSlot) {
          // Remove slot
          const newSlots = existingSession.slot_numbers.filter(
            (s) => s !== slot
          )
          ...
          return {
            ...
            schedule_sessions: prev.schedule_sessions.map((s) =>
              key(s)
                ? { ...s, slot_numbers: newSlots.sort((a, b) => a - b) }
                : s
            ),
          }
        } else {
          ...
          return {
            ...
            schedule_sessions: prev.schedule_sessions.map((s) =>
              key(s)
                ? {
                    ...s,
                    slot_numbers: [...s.slot_numbers, slot].sort(
                      (a, b) => a - b
                    ),
                  }
                : s
        ...
        const otherSession = prev.schedule_sessions.find(
          (s) =>
            s.day_of_week === day &&
            s.session_type !== activeSessionType &&
            s.slot_numbers.includes(slot)
        )
        if (otherSession) {
          // Remove from other session_type first
          const newOtherSlots = otherSession.slot_numbers.filter(
            (s) => s !== slot
          )
          const filtered =
            newOtherSlots.length === 0
              ? prev.schedule_sessions.filter(
                  (s) =>
                    !(
                      s.day_of_week === day &&
                      s.session_type !== activeSessionType
              ...
              : prev.schedule_sessions.map((s) =>
                  s.day_of_week === day && s.session_type !== activeSessionType
                    ? {
                        ...s,
                        slot_numbers: newOtherSlots.sort((a, b) => a - b),
                      }
                    : s
                )
          return {
  ...
  const isSlotSelected = (day: string, slot: number) => {
    return formData.schedule_sessions.some(
      (s) => s.day_of_week === day && s.slot_numbers.includes(slot)
    )
  }

  const getSlotSessionType = (
    day: string,
    slot: number
  ): "theory" | "lab" | null => {
    const session = formData.schedule_sessions.find(
      (s) => s.day_of_week === day && s.slot_numbers.includes(slot)
    )
    ...
    <div className="container mx-auto max-w-5xl px-4 py-6">
                  ...
                  <SelectContent>
                    {availableSemesters.map((s) => (
                      <SelectItem key={s.id} value={s.name}>
                        {s.name}{" "}
                        {s.status === "active" ? "(Aktif)" : "(Planlanan)"}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Dönemlik ders açılacak bölümü seçin
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Açılacak ders, öğretim üyesi ve sınıf bilgilerini girin
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {/* Ders Seçimi */}
            ...
            {selectedCourse && (
              <div className="rounded-lg bg-muted/50 p-4">
                <h4 className="mb-2 font-medium">Seçilen Ders Bilgileri</h4>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Dersin hangi gün ve saatlerde yapılacağını seçin. Tip seçip
              slotlara tıklayın.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {selectedCourse ? (
                ...
                <div className="flex gap-2">
                  {totalTheoryHours > 0 && (
                    <Button
                      ...
                      size="sm"
                      variant={
                        activeSessionType === "theory" ? "default" : "outline"
                      }
                  ...
                  {totalLabHours > 0 && (
                    <Button
                      ...
                      size="sm"
                      variant={
                        activeSessionType === "lab" ? "default" : "outline"
                      }
                      className={
                  ...
                  <div className="ml-4 flex items-center gap-3 text-xs text-muted-foreground">
                    {totalTheoryHours > 0 && (
                      <span className="flex items-center gap-1">
                        <span className="inline-block h-3 w-3 rounded bg-indigo-200 dark:bg-indigo-800" />{" "}
                        Teori
                      </span>
                    )}
                    {totalLabHours > 0 && (
              ...
              <p className="mb-4 text-sm text-muted-foreground">
                Ders seçildikten sonra program oluşturabilirsiniz.
              </p>
            )}
            {existingSemesterCourses.length > 0 && (
              <div className="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-800 dark:bg-blue-950/20">
                <p className="flex items-center gap-2 text-sm text-blue-900 dark:text-blue-100">
                  ...
                  <span>
                    <strong>{existingSemesterCourses.length}</strong> ders bu
                    dönem için açılmış. Kırmızı alanlar dolu saatleri gösterir.
                  </span>
                </p>
              </div>
            )}
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    <th className="border bg-muted p-2 text-left text-sm font-medium dark:border-gray-700">
                      Saat
                    </th>
                    {daysOfWeek.map((day) => (
                      <th
                        key={day.key}
                        className="min-w-[100px] border bg-muted p-2 text-center text-sm font-medium dark:border-gray-700"
                      >
                        {day.label}
                      </th>
                    ))}
                ...
                <tbody>
                  {timeSlots.map((slot) => (
                    <tr key={slot.slot}>
                      <td className="border p-2 text-xs whitespace-nowrap text-muted-foreground dark:border-gray-700">
                        {slot.slot}. Ders
                        <br />
                      ...
                      {daysOfWeek.map((day) => {
                        ...
                        const slotType = selected
                          ? getSlotSessionType(day.key, slot.slot)
                          : null
                        const courseCode = occupied
                          ? getOccupiedSlotCourse(day.key, slot.slot)
                          : null

                        return (
                          <td
                            key={day.key}
                            className={`border p-1 text-center dark:border-gray-700 ${
                              occupied
                            ...
                            title={
                              occupied
                                ? `Dolu: ${courseCode}`
                                : slotType
                                  ? `${slotType === "theory" ? "Teori" : "Lab"}`
                                  : ""
                            }
                          >
                            {occupied ? (
                              <div className="flex h-full items-center justify-center py-2">
                                <span className="text-center text-[10px] leading-tight font-semibold text-red-600 dark:text-red-400">
                                  {courseCode}
                                </span>
                              </div>
                            ) : (
                              <Checkbox
                                checked={selected}
                                onCheckedChange={() =>
                                  toggleScheduleSlot(day.key, slot.slot)
                                }
                                className="h-5 w-5"
                              />
                            )}
            ...
            {formData.schedule_sessions.length > 0 && (
              <div className="mt-4 rounded-lg bg-muted/50 p-3">
                <h4 className="mb-2 text-sm font-medium">Seçilen Saatler:</h4>
                <div className="flex flex-wrap gap-2">
                  {formData.schedule_sessions.map((session) => {
                    const dayLabel = daysOfWeek.find(
                      (d) => d.key === session.day_of_week
                    )?.label
                    const typeLabel =
                      session.session_type === "lab" ? "Lab" : "Teori"
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
              ...
              <div key={index} className="flex items-end gap-3">
                <div className="flex-1 space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateAssessment(index, "slug", value)
                    }
                  >
                    <SelectTrigger>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <div
                className={`flex items-center gap-2 text-sm font-medium ${isWeightValid ? "text-green-600" : "text-destructive"}`}
              >
                {isWeightValid ? (
                ...
                Toplam: %{totalWeight}
                {!isWeightValid && (
                  <span className="text-xs">(%100 olmalı)</span>
                )}
              </div>
            </div>
          </CardContent>
      ...
      {previewDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          />
          {/* Content */}
          <div className="relative z-50 flex max-h-[94vh] w-[96vw] flex-col rounded-xl bg-white shadow-2xl dark:bg-gray-900">
            {/* Header */}
            <div className="shrink-0 border-b px-6 py-4 dark:border-gray-700">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {getDepartmentName(formData.department_id) && (
                  <span>
                    <strong>{getDepartmentName(formData.department_id)}</strong>{" "}
                    bölümünün{" "}
                  </span>
                )}
                ders programını gözden geçirin. Onayladıktan sonra sonraki adıma
                geçilecektir.
              </p>
            </div>
            ...
            <div className="flex-1 space-y-6 overflow-y-auto px-6 py-4">
              {[1, 2, 3, 4].map((classLevel) => {
                const hasEntries = daysOfWeek.some((day) =>
                  timeSlots.some(
                    (slot) =>
                      previewSchedule[classLevel]?.[day.key]?.[slot.slot]
                  )
                )
                      ...
                      <table className="w-full table-fixed border-collapse text-xs">
                        <thead>
                          <tr className="bg-gray-50 dark:bg-gray-700">
                            <th className="w-20 border px-2 py-1.5 font-semibold text-gray-700 dark:border-gray-600 dark:text-gray-300">
                              Saat
                            </th>
                            {daysOfWeek.map((day) => (
                              <th
                                key={day.key}
                                className="border px-2 py-1.5 font-semibold text-gray-700 dark:border-gray-600 dark:text-gray-300"
                              >
                                {day.label}
                              </th>
                            ))}
                        ...
                        <tbody>
                          {timeSlots.map((slot) => (
                            <tr key={slot.slot}>
                              <td className="border bg-gray-50 px-1 py-1 text-center font-medium text-gray-600 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-400">
                                {slot.slot}. Ders
                                <br />
                              ...
                              {daysOfWeek.map((day) => {
                                const entry =
                                  previewSchedule[classLevel]?.[day.key]?.[
                                    slot.slot
                                  ]
                                return (
                                  <td
                                    key={day.key}
                                    className="border p-0.5 dark:border-gray-600"
                                  >
                                    {entry ? (
                                      <div
                                        className={`${entry.color} flex h-[52px] flex-col justify-center rounded border p-1.5`}
                                      >
                                      ...
                                      <div className="flex h-[52px] items-center justify-center rounded bg-orange-50 text-[10px] font-medium text-orange-400 dark:bg-orange-950/20 dark:text-orange-500">
                                        Öğle Arası
                                      </div>
                                    ) : (
            ...
            {/* Footer */}
            <div className="shrink-0 space-y-3 border-t px-6 py-4 dark:border-gray-700">
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-900/20">
                <p className="text-sm text-amber-700 dark:text-amber-300">
                  Onayladıktan sonra ders programı kesinleşecektir. Devam etmek
                  istiyor musunuz?
                </p>
              </div>
