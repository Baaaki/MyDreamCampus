...
async function lookupCourseUUID(courseCode: string): Promise<string> {
  const res = await catalogApiSafe
    .get(`courses/${courseCode.trim().toUpperCase()}`)
    .json<{ id: string }>()
  return res.id
}
...
export function GradesPeriodsTab({
  showToast,
  activeSemester,
}: {
  showToast: (
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (editData.period_end)
        payload.period_end = new Date(editData.period_end).toISOString()
      if (editData.is_active !== undefined)
  ...
  const openEdit = (period: AcademicPeriod) => {
    ...
    setEditData({
      period_end: period.period_end
        ? new Date(period.period_end).toISOString().slice(0, 16)
        : "",
    ...
    <div className="space-y-4 pt-4">
      <div className="mb-4 flex items-end justify-between">
        <div>
          <h3 className="text-lg font-medium">
            Not Servisi ({activeSemester.name})
          </h3>
          <p className="text-sm text-gray-500">
            Hocaların not girebileceği tarih aralıklarını belirleyin.
          </p>
        </div>
            ...
            {loading ? (
              ...
              periods.map((period) => (
                <TableRow key={period.id}>
                  ...
                  <TableCell className="text-sm">
                    {format(new Date(period.period_end), "dd MMM yyyy HH:mm", {
                      locale: tr,
                    })}
                  </TableCell>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Yeni Deadline — Notlar ({activeSemester.name})
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                type="datetime-local"
                value={
                  typeof editData.period_end === "string"
                    ? editData.period_end
                    : ""
                }
                onChange={(e) =>
                  setEditData({ ...editData, period_end: e.target.value })
                }
                className="mt-1"
              />
            </div>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bu deadline kaydını silmek istediğinize emin misiniz? Bu işlem
              geri alınamaz.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export function SimplePeriodsTab({
  serviceKey,
  showToast,
  activeSemester,
}: {
  serviceKey: SimplePeriodServiceKey
  showToast: (
    msg: string,
    type: "error" | "warning" | "success" | "info"
  ) => void
  ...
  const [editDialogOpen, setEditDialogOpen] = useState(false)
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false)
  const [selectedPeriod, setSelectedPeriod] = useState<SimplePeriod | null>(
    null
  )
  ...
  return (
    <div className="space-y-4 pt-4">
      <div className="mb-4 flex items-end justify-between">
        <div>
          <h3 className="text-lg font-medium">
            {serviceLabel === "Katalog/Ders Programı"
              ? "Katalog Servisi"
              : "Ders Kayıt Sistemi"}{" "}
            ({activeSemester.name})
          </h3>
...
export function ClosedDaysTab({
  ...
  if (!activeSemester) {
    return (
      <div className="pt-8 text-center text-gray-500">
        <AlertTriangle className="mx-auto mb-3 h-8 w-8 text-amber-500" />
        <p>
          Kapalı gün (özel tatil) eklemek için lütfen aktif bir dönem açılışı
          yapın.
        </p>
      </div>
    )
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showToast(
        "Kapalı gün eklenemedi (bu tarih zaten kayıtlı olabilir)",
        "error"
      )
    } finally {
      setFormLoading(false)
  ...
  return (
    <div className="space-y-4 pt-4">
      <div className="mb-4 flex items-end justify-between">
        <div>
          <h3 className="text-lg font-medium">
            Yemekhane Servisi ({activeSemester.name})
          </h3>
          <p className="text-sm text-gray-500">
            Öğrencilerin rezervasyon yapamayacağı kapalı günleri tanımlayın.
          </p>
        </div>
        <div className="flex gap-2">
          <Button
