...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await authApi.post("reset-password", { json: { token, new_password: newPassword } });
      setDone(true);
    } catch (err) {
      setError(await apiErrorMessage(err, "Şifre sıfırlanamadı, lütfen tekrar deneyin"));
    } finally {
      setLoading(false);
    }
  };
        ...
        {done ? (
          <div className="rounded-md bg-green-50 p-4">
            <p className="text-sm text-green-800">
              Şifreniz güncellendi. Tüm oturumlarınız kapatıldı; yeni şifrenizle giriş yapabilirsiniz.
            </p>
          </div>
        ) : !token ? (
          <div className="rounded-md bg-red-50 p-4">
            <p className="text-sm text-red-800">
              Sıfırlama bağlantısı eksik veya geçersiz. Lütfen yeni bir bağlantı isteyin.
            </p>
          </div>
        ) : (
