...
export default function GradeEntryPage() {
  ...
  const currentAssessment: AssessmentStatus | undefined =
    courseStatus?.assessments.find((a) => a.slug === slug)
    ...
    try {
      const scores = Array.from(dirtyIds).map((regId) => {
        ...
        return {
          ...
          score:
            local.isAbsent || local.score === ""
              ? null
              : parseInt(local.score, 10),
          is_absent: local.isAbsent,
        }
      })

      const result = await gradesService.bulkSubmitScores(courseId, {
        slug,
        scores,
      })
      ...
      if (result.auto_finalized) {
        message +=
          " Tüm notlar girildiği için ders notları otomatik olarak kesinleştirildi."
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        >
          ...
          {dirtyIds.size > 0
            ? `Kaydet (${dirtyIds.size} değişiklik)`
            : "Kaydet"}
        </Button>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {filteredStudents.map((student, index) => {
              ...
              const scoreInvalid =
                local.score !== "" && !isScoreValid(local.score)
      ...
      <div className="flex items-center justify-between rounded-lg bg-gray-50 px-4 py-3 text-sm dark:bg-gray-800/50">
        <div className="flex items-center gap-1.5 text-gray-600 dark:text-gray-400">
          ...
          <span>
            Toplam: <span className="font-medium">{students.length}</span>{" "}
            öğrenci
          </span>
        </div>
