...
describe("luhnValid", () => {
  it.each(["4242424242424242", "4000000000000002", "4000000000009995", "378282246310005"])(
    "accepts %s",
    (number) => {
  ...
  it.each(["4242424242424241", "1234567812345678", "4242 4242", ""])("rejects %j", (number) => {
...
describe("validateCardForm", () => {
  ...
  it("rejects a number that fails Luhn", () => {
    const errors = validateCardForm({ ...validForm, number: "4242 4242 4242 4241" }, now);
  ...
  it("rejects a CVC that is not three or four digits", () => {
    ...
    expect(validateCardForm({ ...validForm, cvc: "12a" }, now).cvc).toBeDefined();
    expect(validateCardForm({ ...validForm, cvc: "1234" }, now).cvc).toBeUndefined();
  });
});

describe("toConfirmRequest", () => {
  it("sends bare digits and a four-digit year", () => {
    expect(toConfirmRequest({ ...validForm, holder: "  Zeynep Şahin " })).toEqual({
