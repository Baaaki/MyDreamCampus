...
// (main.go) filters on course_catalog.period.grading.*, so only this module's
// period type ever lands here.
const QueuePeriodEvents = "grades.period_events"
...
// PeriodConsumer keeps grades' local academic_periods projection in sync with
// catalog. No processed_events dedup: the upsert keys on semester and the
// delete is by semester, so redelivery after a nack cannot corrupt state.
...
func (w *PeriodConsumer) Start(ctx context.Context) error {
	log := logger.WithContextAndFields(ctx, zap.String("worker", "GradesPeriodConsumer"))
	...
	log.Info("grades period consumer started", zap.String("queue", QueuePeriodEvents))
	return nil
}

func (w *PeriodConsumer) handleMessage(ctx context.Context, body []byte) error {
	log := logger.WithContextAndFields(ctx,
		zap.String("worker", "GradesPeriodConsumer"),
	...
	switch events.PeriodEventAction(env.EventType) {
