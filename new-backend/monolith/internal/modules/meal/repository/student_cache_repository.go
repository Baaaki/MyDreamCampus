...
func (r *StudentCacheRepository) UpsertStudentCache(ctx context.Context, params db.UpsertStudentCacheParams) (db.StudentView, error) {
	student, err := r.queries.UpsertStudentCache(ctx, params)
	if err != nil {
		return db.StudentView{}, fmt.Errorf("%w: failed to upsert student cache: %v", sharedErrors.ErrQueryFailed, err)
	}
	return student, nil
}

// GetStudentCacheByID returns student from cache
func (r *StudentCacheRepository) GetStudentCacheByID(ctx context.Context, id uuid.UUID) (db.StudentView, error) {
	student, err := r.queries.GetStudentCacheByID(ctx, utils.UUIDToPgtype(id))
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return db.StudentView{}, fmt.Errorf("%w", sharedErrors.ErrNotFoundRepo)
		}
		return db.StudentView{}, fmt.Errorf("%w: failed to get student cache: %v", sharedErrors.ErrQueryFailed, err)
	}
	return student, nil
}
