...
ALTER TABLE auth.outbox_events ADD COLUMN correlation_id UUID;

CREATE INDEX idx_auth_outbox_correlation
    ON auth.outbox_events(correlation_id)
    WHERE correlation_id IS NOT NULL;

-- +goose Down
DROP INDEX IF EXISTS auth.idx_auth_outbox_correlation;
ALTER TABLE auth.outbox_events DROP COLUMN correlation_id;
