...
// (main.go) filters on course_catalog.period.enrollment.*, so only this
// module's period type ever lands here.
const QueuePeriodEvents = "enrollment.period_events"
...
// PeriodConsumer keeps enrollment's local academic_periods projection in sync
// with catalog. No processed_events dedup: the upsert keys on semester and the
...
func (w *PeriodConsumer) Start(ctx context.Context) error {
	log := logger.WithContextAndFields(ctx, zap.String("worker", "EnrollmentPeriodConsumer"))
	...
	log.Info("enrollment period consumer started", zap.String("queue", QueuePeriodEvents))
	return nil
}

func (w *PeriodConsumer) handleMessage(ctx context.Context, body []byte) error {
	log := logger.WithContextAndFields(ctx,
		zap.String("worker", "EnrollmentPeriodConsumer"),
