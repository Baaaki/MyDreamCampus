...
ALTER TABLE meal.outbox_events ADD COLUMN correlation_id UUID;

CREATE INDEX idx_meal_outbox_correlation
    ON meal.outbox_events(correlation_id)
    WHERE correlation_id IS NOT NULL;

-- +goose Down
DROP INDEX IF EXISTS meal.idx_meal_outbox_correlation;
ALTER TABLE meal.outbox_events DROP COLUMN correlation_id;
